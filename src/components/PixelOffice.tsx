...
const agents: Agent[] = [
  ...
  { id: 'roger', name: 'Roger', role: 'Infrastructure', dept: 'Engineering', status: 'online', zone: 'engineering', initial: 'Ro', color: '#f97316' },
  ...
  { id: 'vegeta', name: 'Vegeta', role: 'LinkedIn', dept: 'Marketing', status: 'online', zone: 'conference', initial: 'V', color: '#8b5cf6' },
  ...
  { id: 'nico', name: 'El Father', role: 'Academic (Nico)', dept: 'Contractors', status: 'online', zone: 'help-desk', initial: 'E', color: '#f59e0b' },
...
// Zone definitions with colors and furniture - SPACED OUT, NO OVERLAP
const zones = {
  exec: { 
    ...
    height: 3,
    furniture: [
      { sprite: 'desk_SE', x: 1.2, y: 1.2 },
      { sprite: 'lampSquareFloor_SE', x: 0.3, y: 0.3 },
    ]
  },
  engineering: { 
    name: 'Engineering', 
    ...
    height: 4,
    furniture: [
      { sprite: 'desk_SE', x: 0.5, y: 0.5 },
      { sprite: 'desk_SW', x: 3.8, y: 0.5 },
      { sprite: 'desk_SE', x: 0.5, y: 2.8 },
      { sprite: 'desk_SW', x: 3.8, y: 2.8 },
      { sprite: 'cabinetBed_SE', x: 4.5, y: 0.2 },
    ]
  },
  design: { 
    name: 'Design Studio', 
    ...
    height: 3,
    furniture: [
      { sprite: 'tableCloth_SE', x: 1.5, y: 1 },
      { sprite: 'chairModernCushion_SE', x: 0.8, y: 0.8 },
      { sprite: 'chairModernCushion_SW', x: 3.2, y: 1.2 },
      { sprite: 'lampSquareTable_SE', x: 1.5, y: 2.2 },
    ]
  },
  cafe: { 
    name: 'Cafe', 
    ...
    height: 3,
    furniture: [
      { sprite: 'tableCoffeeGlassSquare_SE', x: 1.5, y: 1.5 },
      { sprite: 'loungeChair_NE', x: 0.5, y: 0.5 },
      { sprite: 'loungeChair_NW', x: 2.3, y: 2.3 },
    ]
  },
  conference: { 
    ...
    height: 4,
    furniture: [
      { sprite: 'tableCloth_SE', x: 2.5, y: 2 },
      { sprite: 'chairCushion_NE', x: 0.8, y: 0.8 },
      { sprite: 'chairCushion_NW', x: 4.2, y: 0.8 },
      { sprite: 'chairCushion_SE', x: 0.8, y: 3 },
      { sprite: 'chairCushion_SW', x: 4.2, y: 3 },
    ]
  },
  'help-desk': { 
    name: 'Help Desk', 
    ...
    height: 4,
    furniture: [
      { sprite: 'desk_SE', x: 0.8, y: 0.5 },
      { sprite: 'desk_SE', x: 0.8, y: 2.8 },
    ]
  },
  operations: { 
    name: 'Operations', 
    ...
    height: 3,
    furniture: [
      { sprite: 'desk_SE', x: 0.6, y: 0.6 },
      { sprite: 'desk_SE', x: 2.2, y: 2.2 },
    ]
  },
};
...
const OFFICE_SCALE = 1.5; // ZOOM IN 1.5x to show full office in one view
...
// Desk positions within each zone (relative to zone origin) - SPREAD OUT FOR NO STACKING
const deskPositions = {
  exec: [
    { x: 1.5, y: 1.5 }, // Rick's executive desk
  ],
  engineering: [
    { x: 0.8, y: 0.8 }, // Desk 1 - SPREAD ACROSS DIFFERENT POSITIONS
    { x: 3.5, y: 0.8 }, // Desk 2 - FAR RIGHT
    { x: 0.8, y: 3.0 }, // Desk 3 - BOTTOM LEFT
    { x: 3.5, y: 3.0 }, // Desk 4 - BOTTOM RIGHT
    { x: 2.0, y: 2.0 }, // Desk 5 - CENTER (Larry)
  ],
  design: [
    { x: 1.0, y: 1.0 }, // Desk 1
    { x: 3.0, y: 1.5 }, // Desk 2 - SPACED OUT
  ],
  cafe: [
    { x: 0.5, y: 0.8 }, // Casual spot 1
    { x: 2.2, y: 0.5 }, // Casual spot 2
    { x: 0.8, y: 2.0 }, // Casual spot 3
    { x: 2.5, y: 2.2 }, // Casual spot 4
    { x: 1.5, y: 1.5 }, // Casual spot 5
  ],
  conference: [
    { x: 1.0, y: 1.0 }, // Seat 1 - TOP LEFT
    { x: 2.5, y: 1.0 }, // Seat 2 - TOP CENTER
    { x: 4.0, y: 1.0 }, // Seat 3 - TOP RIGHT
    { x: 1.0, y: 2.8 }, // Seat 4 - BOTTOM LEFT
    { x: 2.5, y: 2.8 }, // Seat 5 - BOTTOM CENTER
    { x: 4.0, y: 2.8 }, // Seat 6 - BOTTOM RIGHT
  ],
  'help-desk': [
    { x: 1.0, y: 0.8 }, // Support station 1
    { x: 1.0, y: 2.5 }, // Support station 2
    { x: 2.0, y: 1.5 }, // Support station 3 - EXTRA SPOTS
    { x: 1.5, y: 3.2 }, // Support station 4
  ],
  operations: [
    { x: 0.8, y: 0.8 }, // Desk 1
    { x: 2.2, y: 2.2 }, // Desk 2 - SPREAD DIAGONALLY
  ],
};
...
export default function PixelOffice() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      officeContainer.scale.set(OFFICE_SCALE); // ZOOM IN 1.5x
      officeContainer.x = 600;
      officeContainer.y = 100;
      app.stage.addChild(officeContainer);

      // Draw background floor tiles to fill black space
      ...
      for (let gy = -2; gy < 18; gy++) {
        for (let gx = -2; gx < 20; gx++) {
          ...
          floorTile.fill({ color: 0x1a1a1a, alpha: 1 });
          floorTile.stroke({ color: 0x27272a, alpha: 0.3, width: 1 });
